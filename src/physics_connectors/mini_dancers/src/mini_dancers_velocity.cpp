...
private:
    ...
    dancers_update_proto::DancersUpdate StepSimulation(dancers_update_proto::DancersUpdate update_msg) override
    {
        ...
        if (!update_msg.payload().empty())
        {
            ...
            velocity_messages.ParseFromString(gzip_decompress(update_msg.payload()));
            // At each timestep, we receive the velocity command of all the agents. Update their velocity/yaw controller respectively
            for (const auto& velocity : velocity_messages.pose())
            {
                std::unique_lock lock(this->agents_mutex_);
                auto agent = this->agents_.find(velocity.agent_id());
                if (agent != this->agents_.end())
                {
                    reference::VelocityHdg controller;
                    controller.velocity[0] = velocity.vx();
                    controller.velocity[1] = velocity.vy();
                    controller.velocity[2] = velocity.vz();
                    // For now, all the UAVs face the same direction and we use their holonomic nature to perform flocking
                    controller.heading = 0.0;
                    agent->second->uav_system->setInput(controller);
                    RCLCPP_DEBUG(this->get_logger(), "Received velocity command for agent %d: (%f, %f, %f)", velocity.agent_id(), velocity.x(), velocity.y(), velocity.z());
                }
                else
                {
                    RCLCPP_WARN(this->get_logger(), "Received velocity command for unknown agent %d", velocity.agent_id());
                }
            }
        }
        else
