...
// Old parameters
    ...
    // **************** MOBILITY MODULE ****************
            ...
            for(uint32_t i = 0; i < numNodes; i++){
                ...
                if(i == 0){
                    ap.Get(0)->AggregateObject(nodeMob);
                } else {
                    stas.Get(i-1)->AggregateObject(nodeMob);
                }
            }
    ...
    //         {
    //             if(i == 0){
    //                 Ptr<Socket> recvSink = Socket::CreateSocket(node_ap.Get(i), tid);
    //                 InetSocketAddress local = InetSocketAddress(Ipv4Address::GetAny(), 80);
    //                 recvSink->Bind(local);
    //                 recvSink->SetRecvCallback(MakeCallback(&ReceivePacket));
    //                 node_ap.Get(i)->AggregateObject(recvSink);
    //                 ip_node_map.emplace(ip.GetAddress(i), nodes.Get(i));
    //                 std::cout << "IP of node " << i << " : " << ip.GetAddress(i) << std::endl;
    //             } else {
    //                 Ptr<Socket> recvSink = Socket::CreateSocket(nodes_sta.Get(i-1), tid);
    //                 InetSocketAddress local = InetSocketAddress(Ipv4Address::GetAny(), 80);
    //                 recvSink->Bind(local);
    //                 recvSink->SetRecvCallback(MakeCallback(&ReceivePacket));
    //                 nodes_sta.Get(i-1)->AggregateObject(recvSink);
    //                 ip_node_map.emplace(ip.GetAddress(i-1), nodes_sta.Get(i-1));
    //                 std::cout << "IP of node " << i << " : " << ip.GetAddress(i-1) << std::endl;
    //             }
    //         }
            ...
            // packetSocket.Install (nodes_sta);
            // packetSocket.Install (node_ap);

            // PacketSocketAddress sock;
            // sock.SetSingleDevice (devices.Get (0)->GetIfIndex ());
            // sock.SetPhysicalAddress (ap.Get (0)->GetAddress ());
            // sock.SetProtocol (1);

            // OnOffHelper onoff ("ns3::PacketSocketFactory", Address (sock));
            // onoff.SetConstantRate (DataRate ("500kb/s"));

            // ApplicationContainer apps = onoff.Install (nodes_sta.Get (0));
    ...
    // **************** MAIN SIMULATION LOOP ****************
            while(true){
                ...
                // for(int i=0; i < numNodes; i++){
                //     RCLCPP_INFO(this->get_logger(), "Node %d received %d packets", i , this->numReceivedPackets[i]);
                // }
