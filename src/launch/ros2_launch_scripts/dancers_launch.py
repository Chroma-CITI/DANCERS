...
def generate_launch_description():
    ...
    setup_network = ExecuteProcess(
        cmd=['sudo', ros_ws + '/src/launch/other_scripts/setup_network.bash', str(config['robots_number'])],
        name='setup_network'
    )
    ...
    spawn_px4 = ExecuteProcess(
        cmd=[ros_ws + '/src/launch/other_scripts/spawn_px4_netns.bash', str(config['robots_number']), str(config['robots_model'])],
        name='spawn_px4',
        env=node_env
    )
